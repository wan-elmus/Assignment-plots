...
mydata.ub.train <- na.omit(mydata.ub.train)
...
mydata.b.train <- mydata.b.train %>%         
                                    mutate(Class = factor(Class, labels = c("NonMal","Mal")))
mydata.b.train <- na.omit(mydata.b.train)
...
predictors_cleaned <- na.omit(mydata.ub.train[,predictors])
outcome_cleaned <- na.omit(mydata.ub.train[,outcome])
model1_ub <- cv.glmnet(as.matrix(predictors_cleaned), as.factor(outcome_cleaned), family="binomial", alpha=0.5)
model2_ub <- randomForest(as.factor(outcome_cleaned) ~ ., data=predictors_cleaned, ntree=500, mtry=3)
...
# model1_ub <- cv.glmnet(as.matrix(mydata.ub.train[,predictors]), as.factor(mydata.ub.train[,outcome]), family="binomial", alpha=0.5)
model1_ub <- cv.glmnet(as.matrix(predictors_cleaned), as.factor(outcome_cleaned), family="binomial", alpha=0.5)

# Train random forest model on unbalanced dataset
# model2_ub <- randomForest(as.factor(mydata.ub.train[,outcome]) ~ ., data=mydata.ub.train[,predictors], ntree=500, mtry=3)
model2_ub <- randomForest(as.factor(outcome_cleaned) ~ ., data=predictors_cleaned, ntree=500, mtry=3)


# Train logistic elastic-net regression model on balanced dataset
model1_b <- cv.glmnet(as.matrix(predictors_cleaned), as.factor(outcome_cleaned), family="binomial", alpha=0.5)

# Train random forest model on balanced dataset
model2_b <- randomForest(as.factor(outcome_cleaned) ~ ., data=predictors_cleaned, ntree=500, mtry=3)
